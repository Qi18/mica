...
def init_model(args):
    ...
    if Path(args.load_from).resolve() == (Path(__file__).resolve().parents[1] / "tokenizer").resolve():
        model = MicaForCausalLM(MicaConfig(hidden_size=args.hidden_size, num_hidden_layers=args.num_hidden_layers, use_moe=bool(args.use_moe)))
...
def main():
    ...
    parser.add_argument('--load_from', default='../tokenizer', type=str, help="模型加载路径（tokenizer=原生torch权重，其他路径=transformers格式）")
