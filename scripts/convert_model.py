...
def convert_torch2transformers_minimind(torch_path, transformers_path, dtype=torch.float16):
    MicaConfig.register_for_auto_class()
    MicaForCausalLM.register_for_auto_class("AutoModelForCausalLM")
    lm_model = MicaForCausalLM(lm_config)
...
def convert_merge_base_lora(base_torch_path, lora_path, merged_torch_path):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    lm_model = MicaForCausalLM(lm_config).to(device)
...
if __name__ == '__main__':
    lm_config = MicaConfig(hidden_size=768, num_hidden_layers=8, max_seq_len=8192, use_moe=False)
    ...
    # convert_json_to_jinja('../tokenizer/tokenizer_config.json', '../tokenizer/chat_template.jinja')
    # convert_jinja_to_json('../tokenizer/chat_template.jinja')