...
def main() -> None:
    ...
    for path in files:
        ...
        if path.suffix.lower() == ".md" and relative.parts[0] != "archive":
    ...
    for required in (ROOT / "LICENSE", ROOT / "NOTICE"):
        if not required.is_file():
            failures.append(f"missing attribution asset: {required.relative_to(ROOT)}")
